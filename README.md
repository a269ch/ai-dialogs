...
- 🔄 **Cross-Agent Dialogue Migration & Transfer Engine**:
  - Seamlessly convert and transfer entire dialogues between any agents (e.g. Claude Code → Antigravity, Antigravity → OpenAI Codex).
...
| `-p`, `--provider <AGENT>` | Filter dialogues by agent (`agy`, `claude`, `codex`, `grok`, `universal`) |
| `-l`, `--list` | Print dialogues table to console |
...
| `--export-json <PATH>` | Export dialogue to canonical JSON file (requires ID as argument) |
...
ai-dialogs 4991ce29 --export-json ~/Desktop/my_dialogue.json
